...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE transactions SET category_id = ?, type = ?, amount = ?, description = ?, transaction_date = ?
        ...
        <form method="POST" action="edit_transaction.php?id=<?php echo $id; ?>">
            ...
            <select name="category_id">
                <?php while ($cat = mysqli_fetch_assoc($categories)) { ?>
                    <option value="<?php echo $cat['category_id']; ?>" <?php if ($cat['category_id'] == $t['category_id']) echo 'selected'; ?>>
